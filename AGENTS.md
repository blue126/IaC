...
- OpenCode Desktop must run in a long-lived attached session. Use mode-specific Sandbox names so an old mapping cannot be mistaken for a new mode. Default loopback command: `sbx run --name iac-opencode-desktop-TASK-loopback-v130 --publish 127.0.0.1:4096:4096 opencode . --kit ./.sandbox-kit -- serve --hostname 0.0.0.0 --port 4096`. The only LAN exception is `sbx run --name iac-opencode-desktop-TASK-lan-v130 --publish <LAN_IP>:4096:4096 opencode . --kit ./.sandbox-kit -- serve --hostname 0.0.0.0 --port 4096`, and only when the user explicitly requests access from a trusted, isolated private LAN.
  ...
  - LAN mode requires a non-empty, strong random `OPENCODE_SERVER_PASSWORD` of at least 20 characters in the OpenCode server process environment, injected through a runtime channel that is not version controlled. Never put the value in repository files, examples, command arguments, or shell history. Plain HTTP is permitted only on that trusted isolated LAN; any other network path requires TLS or a trusted VPN.
  - After starting LAN mode, verify the host listens only on the selected `<LAN_IP>`, unauthenticated access fails, and authenticated access succeeds. If any post-check fails, immediately terminate the attached session and confirm the host port is closed.
