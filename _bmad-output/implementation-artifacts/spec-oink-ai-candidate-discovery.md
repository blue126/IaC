...
- 2026-09-08 human renegotiation: a strict corpus run proved that the Action completed successfully only after three turns but withheld structured output because the prior two-turn limit rejected it. The bound is raised to 3; only trusted-base `Read` remains allowed, and the corpus restarts as a new version.
- 2026-09-07 human renegotiation: the known-working `claude-review` Action establishes that Claude Code structured output needs a successful read/tool turn. The model is therefore granted only `Read` from the trusted merge-base checkout; the fixed prompt narrows that read to manifest-named paths and keeps sensitive/production paths forbidden.
