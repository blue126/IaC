...
**持续讨论使用专家自己的线程。** 只 @人类不能改变用户下一条回复的默认 parent。Coordinator 派工时要求专家复用当前工作项中本人发起的顶层评论线程；不存在时由专家以自己的身份发布一条普通顶层评论（省略 --parent），真实 @用户，简述承接来源、进度与下一问。后续问答使用该专家线程的真实根 ID，不每轮建线程。原 Issue 与 Squad 归属不变，不重启原生工作流、不复制状态文件。正式交付、换角色或需协调的阻塞再显式交回 Coordinator。旧线程只保留，不删除或冒充作者；未发送草稿不自动搬移或发送。用户在新专家线程回复，发送前确认不带 @ 的预览仅启动该专家；旧 Coordinator 输入框不会自动改收件人。
