...
## 10. Homepage 集成故障排查 (Troubleshooting)

### 10.1 LXC 容器 DNS 解析失败
**现象**:
*   在 LXC 容器中执行 `apt update` 或 `ping google.com` 失败。
*   `/etc/resolv.conf` 被 Proxmox 覆写为 Tailscale MagicDNS 地址 (如 `100.100.100.100`)，但容器内未运行 Tailscale 或未正确配置。

**原因**:
*   如果 Proxmox 宿主机安装了 Tailscale 并开启了 MagicDNS，Proxmox 会将宿主机的 DNS 配置同步给 LXC 容器。
*   对于未加入 Tailscale 网络或未配置 `/dev/net/tun` 透传的容器，这些 DNS 服务器不可达。

**解决方案**:
1.  **防止覆写**: 在容器内创建 `/etc/.pve-ignore.resolv.conf` 文件。
    ```bash
    touch /etc/.pve-ignore.resolv.conf
    ```
2.  **手动指定 DNS**: 修改 `/etc/resolv.conf` 使用公共 DNS。
    ```bash
...
### 10.2 Homepage Proxmox Widget 显示 "Unknown"
**现象**:
*   Homepage 仪表盘中 Proxmox Widget 显示 "Unknown" 或 "API Error"。
*   容器日志报错 `HTTP Error 403 calling Proxmox API`。
*   `curl` 测试 `/api2/json/version` 成功，但 `/api2/json/nodes/pve0/status` 失败。

**原因**:
*   **Privilege Separation (权限分离)**: Proxmox API Token 默认开启此选项。开启后，Token **不继承** 关联用户的权限。
*   即使关联用户是 `root@pam`，Token 本身也需要显式分配权限。

**解决方案**:
1.  **方法一 (推荐)**: 在 Proxmox GUI (Datacenter > Permissions > API Tokens) 中，选中 Token，点击 "Permissions"，添加 `PVEAuditor` 角色 (路径 `/` 或 `/nodes`)。
2.  **方法二 (不推荐)**: 编辑 Token 时取消勾选 "Privilege Separation"，Token 将继承用户的所有权限。

### 10.3 LXC 容器重启必要性
**现象**:
*   Ansible 修改了 LXC 配置文件 (如 `/etc/pve/lxc/103.conf`) 添加 bind mount，但容器内不生效。

**原因**:
*   LXC 配置文件仅在容器启动时读取。运行时修改不会立即生效。

**解决方案**:
*   在 Ansible Playbook 中，当检测到 LXC 配置变更时，必须触发容器重启。
    ```yaml
