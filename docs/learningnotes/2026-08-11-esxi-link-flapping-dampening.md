...
本次排查连续出现多个错误结论，均源于**基于不完整信息下判断**：

| 错误结论 | 实际 | 教训 |
|---|---|---|
| "管理口绑在没接线的铜口上" | 管理网确在光口 `vmnic2`；ESXi 的 vmk0 会继承安装时物理网卡的 MAC，之后换上联也不变，故 Dell OUI 的 MAC 出现在 Mellanox 光口上属正常 | MAC 的 OUI 不能用来推断当前物理路径 |
| "DHCP 池缩小导致 `.251` 分不出来" | `.249`/`.250`/`.251` 均为**手工配置的静态 IP**，不走 DHCP，池大小无关 | 先确认地址分配方式再推理 |
| "CPU2 的 PCIe 被 BIOS 禁用" | `lspci` 输出被 `head -25` 截断；实际 `a0:05.4` 是 CPU2 的 I/O APIC，且 llm-server 的直通配置里有一张 RTX 3090 挂在 bus `0xa1`（CPU2 侧） | **不要基于截断的输出下结论** |
| "T7910 大概率没有传统 PCI 槽" | Dell 官方文档明确列出 **slot 5 为 PCI card slot** | 涉及硬件规格应先查厂商文档，而非依赖平台通例 |
