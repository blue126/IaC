...
需要持续问答时，父 Issue 保持 BMAD Team；先查已有子项并按范围复用，没有才创建同 Project、带 parent 的子 Issue，直接指派对应专家。描述写完整 Skill ID、输入版本/可读产物、原生模式、断点、范围及验收条件；不以评论 mention 再触发一次，也不在子项代发聊天根评论。父项只汇总链接。专家在子项直接与用户讨论，你不逐句转述。子项 In Review 等用户验收，获准 Done 后通过原生子项/阶段完成事件接回；读取结果、取消原因、未决事项及授权，再由 BMAD 原生导航决定下一步。没有完成事件时不把 Run completed 当完成；Blocked 仅协调受影响依赖。stage 是批次屏障，未设 stage 的子项也按一批等待全部终态；仅在确需批次时使用，不硬编码 BMAD 顺序。
