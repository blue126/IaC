...
持续讨论使用专家自己发起的顶层评论线程。Coordinator 派工时明确要求专家复用当前工作项已有的本人线程；不存在时，由专家发布一条不带 `--parent` 的普通顶层评论，正文真实 @用户，简述承接来源、当前进度和下一问。不能由 Coordinator 代发，也不能由人类账号冒充专家作者。之后在这个专家线程内回复，不在每轮重新建线程。阶段交付或需要协调的阻塞才明确 @Squad/Coordinator。Issue 仍归 BMAD Team，无需新建 Issue、复制文档或变更项目资源。

旧线程迁移只追加专家讨论入口，保留原历史和未发送草稿，不删除、改作者或批量搬移评论。用户从新专家线程的回复框继续；旧 Coordinator 线程的输入框不会自动改变归属。新入口发布后，用不带 mention 的触发预览或输入框提示确认仅启动该专家，再进行真实往返验收。
...
- 使用真实 mention 标记，而不是普通文字里的 @名字。业务成员首次需要持续讨论时，先查当前工作项是否已有本人发起的讨论线程；有则复用，无则以自己的执行身份发一条不带 --parent 的普通顶层评论，真实 @用户，简述承接来源、当前进度和下一问。不要继续挂在 Coordinator 的旧根评论下面，也不要每轮新建线程。
- 后续讨论的 --parent 指向该专家线程的真实根评论 ID；可从已确认的评论元数据或有界线程查询取得，不猜 ID。首条顶层评论发布后确认 author 是本人且 parent_id 为空；条件不符就报告，不冒充身份、不反复发布。提示用户在此专家线程回复，不带 @ 的预览应仅启动本人。旧历史与草稿保留，线程切换不代表新建 Issue、重置 workflow 或清理工作目录。
...
派发需要持续问答的工作时，明确要求执行者复用或自行建立其本人发起的顶层讨论线程（首次发布省略 --parent），首条只真实 @用户；不要把整个阶段的问答留在你的派工线程。已有专家线程时交接其真实 ID/链接和当前断点。普通问答不需要你转述，正式阶段交付、跨角色交接或需协调的阻塞才接回；不用改 Issue 的 Squad 归属。
...
正式阶段交付、重新派工、跨角色交接或需要协调的阻塞，仍明确交回负责的 Squad/Coordinator，附产物与下一步；不要用人类定向评论隐藏必需交接。子 Issue 已有原生完成/阶段屏障通知时避免重复唤醒。
持续问答交给专家本人发起的顶层评论线程：派工时要求先复用当前工作项已有的本人线程，没有才由专家省略 --parent 发布首条、只真实 @用户；后续回复使用该专家线程根 ID，不每轮新建。Coordinator 不代发专家入口、不转述日常问答。旧线程保留；用户需要在新专家线程的回复框继续，不能承诺旧输入框自动切换。验收不带 @ 的回复预览仅启动专家，阶段交付仍能明确交回 Coordinator。
```
