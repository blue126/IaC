...
### 1. 创建 Inventory 配置
**文件**: `inventory/pve_vms/samba.yml`
...
新增/修改的文件：
```
inventory/pve_vms/samba.yml                    # Samba VM 配置（标准化变量命名）
...
修改的文件：
```
inventory/pve_vms/immich.yml                   # 标准化变量命名（去除 immich_vm_* 前缀）
inventory/groups.yml                        # 添加 application_vms 组定义
