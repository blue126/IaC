...
> 上表是 2026-08-05/06 事故恢复后的历史实测基线，保留 109/110 是为了维持事故证据链。2026-08-12 已永久退役这两个 guest 及其全部 PBS 恢复点；2026-09-11 原 Veeam worker 108 与手工 PNET4.2.4 110 已退役，新 `ubuntu-2604` 108 已由 Terraform 创建并验收。

> **当前状态（2026-09-11）**：108 为 `ubuntu-2604`（Ubuntu 26.04 VM），109 为 `mcp-gateway`，110 已释放。VM部署已验收；生产备份集合仍为100–107，新增108待同步。历史事故基线及2026-08-12退役记录保持不变。
...
### 8.6 VMID 108/110 重建（2026-09-11，VM部署已验收）

- 旧108正常关机；110经PVE关机超时后由用户在guest内关机。核实停止状态、磁盘及无HA/复制关系后，两者经精确 `qm destroy` 删除，无purge或孤儿磁盘清理，原磁盘已确认消失，既有备份未删除。
- 旧108的bpg删除请求遇到即时超时；用户批准改用原生命令删除，再以仅包含旧记录移除的HCP refresh-only计划同步。未手改state，未使用 `state rm`。
- 新108由Terraform创建：Ubuntu26.04、4 CPU、32768 MiB、`192.168.1.108/24`；DNS/网关 `192.168.1.1`。系统盘初始20 GiB，随后按用户授权通过仅含disk.size变更的Terraform计划在线扩至100 GiB；guest的 `/dev/sda1` 和ext4同步扩展，根文件系统约96 GiB，全程未重启。镜像已通过SHA256校验，不依赖模板或自定义snippet。
- `deploy-ubuntu-2604.yml` 已成功执行（20项成功、0失败）：复用common基线、安装QGA、独立验证密钥连接及sudo后禁用SSH密码/交互认证。仅允许PVE原生Cloud-Init的已知旧式user字段弃用警告，其他初始化错误仍阻止部署。
- VM108定向Terraform计划返回 `No changes`。全量计划中的pve1不可达、109重建和Homepage漂移未执行。
- `pbs_backup_vmids` 源码新增108，但生产作业仍为100–107；PBS `.249:8007` 不可达，新增备份尚未同步或验证。
...
| 5 | **VMID 108 `veeam-worker` 的来历与去留** | 已于2026-09-11按用户授权退役；新Ubuntu26.04 VM108已创建并验收，见§8.6。 |
...
**pve0 线上作业（历史状态与当前状态）**：2026-08-06 备份作业曾由 100–106 改为 100–107,109,110。2026-08-12 仓库与生产作业均已收缩为 100–107；109/110 guest 及其对应 PBS 恢复点均已删除并核验。2026-09-11 实测活动备份集合仍为 100–107；新 VM108 已验收，但新增备份作业成员仍待单独同步。PBS 不可达期间不宣称新备份已生效。
