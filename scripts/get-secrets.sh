    ...
    - name: Generate Proxmox secrets.auto.tfvars
      copy:
        ...
        content: |
          ...
          # Keep secret generation usable before the pve2 token is provisioned.
          # Full-workspace planning requires both nonempty values; Terraform
          # validation reports the missing credentials rather than using '='.
          pm_api_token_id_pve2     = "{{ vault_proxmox_api_token_id_pve2 | default('') }}"
          pm_api_token_secret_pve2 = "{{ vault_proxmox_api_token_secret_pve2 | default('') }}"
