...
从**宿主任务 worktree**运行 `.claude/skills/run-iac/driver.sh`，由它创建专用 Sandbox，并通过 `sbx exec` 驱动同目录的 `smoke.sh`。以下路径均相对仓库根目录。项目没有统一的应用 GUI；此入口验证真实 Terraform/Ansible CLI，不启动交互 Agent，不需要截图。HCP Terraform 使用宿主 `${HOME}/.terraform.d/credentials.tfrc.json`，该目录只读挂载到新建 Sandbox；凭据缺失时 Driver 会在创建前停止。

## Run（Agent 与人工共用入口）

**先获得用户对创建 Sandbox、在其中执行 Kit 依赖安装，以及只读挂载 HCP Terraform 凭据目录的明确授权。** 参数只是防误触确认，不能代替用户授权。
...
2. 确认宿主 `${HOME}/.terraform.d/credentials.tfrc.json` 可读；缺失时在创建 Sandbox 前停止，不搜索其他凭据位置或执行 `terraform login`。
3. 校验 Kit；用 `sbx create --kit` 创建全新 Claude 模板 Sandbox，不复用旧实例、不发布端口，并将 `${HOME}/.terraform.d` 只读挂载。
4. 为该 Sandbox 建立仅限 `192.168.1.0/24` 的网络 allow rule。
5. 在 Sandbox 的临时目录生成独立 `ansible.cfg`；验证 Terraform、Ansible、pip、Python imports 和全部 7 个必需 collections。
6. 执行 Terraform console 的本地表达式，并以临时 `TF_DATA_DIR` 对现有 Proxmox cloud backend 执行只读 lockfile 的 `terraform init`，再对 **Sandbox localhost** 执行 Ansible ping。
7. 输出 `HCP_TERRAFORM_AUTH_OK` 与 `IAC_SANDBOX_SMOKE_OK`，停止本次创建的 Sandbox。失败时保留非零退出码，也尝试停止已成功创建的实例。

成功标准：看到 HCP 认证 marker、`ping: pong` 对应的 JSON 输出、成功 marker、停止确认，且 driver 退出码为 `0`。仅有 `Created sandbox` 不算验证成功。
...
- 此 smoke 不读取项目 inventory、不解密 Vault、不运行 Terraform plan/apply，也不部署任何远端主机。它会使用只读 HCP Terraform CLI 凭据运行 `terraform init` 来验证现有 Proxmox cloud backend；不需要 OCI 挂载或 SSH 私钥。凭据值不得写入仓库、命令参数、日志或 shell history。
...
- **需要 SSH key 认证的远端任务**：在实际 Ansible 操作前，于 Sandbox 内运行 `ssh-add -L` 确认 forwarded agent 有批准的身份。若目标使用 Ansible Vault 密码登录（例如 `ubuntu` VM 用户），此检查不适用。绝不挂载或复制 `~/.ssh` 私钥目录。
