...
The inline manifest is the primary input. If a read is necessary, read only a path named by its `document` or `evidence` fields from the trusted merge-base checkout. Never read `.env`, Vault, tfvars/state, Notion, production paths, or any path absent from the manifest. Do not write, run commands, use the network, or inspect PR-head files.
